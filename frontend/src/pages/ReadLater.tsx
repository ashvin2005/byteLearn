...
const ReadLaterArticle = () => {
  ...
  useEffect(() => {
    const fetchReadLaterArticles = async () => {
      try {
        ...
        // Fetch user's read later articles
        ...
        // Get the article IDs from the read later articles
        const readLaterIds = userData.watch_later.map(
          (article: ReadLaterArticle) => article.article_id
        );
        ...
        // Filter articles to only include the read later ones
        const readLaterArticlesData =
          articlesData?.filter((article) =>
            readLaterIds.includes(article.article_id || "")
          ) || [];

        setReadLaterArticles(readLaterArticlesData);
      } catch (err) {
        console.error("Error fetching read later articles:", err);
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-black p-11 text-[#D4D4D4]">
        Please log in to view your read later articles.
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-black p-11 text-[#D4D4D4]">
        Error loading read later articles:
        <br />
        {error.message}
      </div>
    );
      ...
      <div className="px-3 md:px-4 pt-8 max-w-[1400px] mx-auto w-full">
        ...
        {readLaterArticles.length === 0 ? (
          <div className="text-lg text-muted-foreground p-4">
            No read later articles found - start bookmarking articles that
            interest you!
          </div>
        ) : (
